...
[[nodiscard]] int calculate() noexcept
{
	return 42;
}
...
{
public:
	[[nodiscard]] int add(int a, int b) const noexcept
	{
		return a + b;
	}
};
...
{
	std::cout << name << " is trivially copyable? " << (std::is_trivially_copyable_v<T> ? "Yes" : "No") << '\n';
}
...
int main()
{
	...
	std::cout << "Operation state: " << (state == OperationState::Success ? "Success" : "Failure") << '\n';
