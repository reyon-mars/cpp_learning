...
namespace utils
{
	...
	void print_status(std::string_view status)
	{
	...
	inline void toggle_debug(Config& cfg) noexcept
	{
		cfg.debug = !cfg.debug;
	}
	inline void record_run(Stats& stats) noexcept
	{
		++stats.runs;
	}
	...
	[[nodiscard]] constexpr int multiply(int a, int b) noexcept
	{
		return a * b;
	}
	[[nodiscard]] constexpr int increment(int x) noexcept
	{
		return x + 1;
	}
	...
	[[nodiscard]] constexpr int absolute(int x) noexcept
	{
		return x < 0 ? -x : x;
	}
	[[nodiscard]] constexpr double average(int a, int b) noexcept
	{
		return (a + b) / 2.0;
	}
	...
	[[nodiscard]] constexpr bool is_positive(int x) noexcept
	{
		return x > 0;
	}
	[[nodiscard]] constexpr bool is_even(int x) noexcept
	{
		return x % 2 == 0;
	}
	[[nodiscard]] constexpr bool is_odd(int x) noexcept
	{
		return x % 2 != 0;
	}
	[[nodiscard]] constexpr bool is_zero(int x) noexcept
	{
		return x == 0;
	}
	[[nodiscard]] constexpr bool is_valid_number(int x) noexcept
	{
		return x >= 0 && x <= 100;
	}

	[[nodiscard]] bool is_empty(std::string_view str) noexcept
	{
		return str.empty();
	}

	[[nodiscard]] std::size_t char_count(std::string_view str) noexcept
	{
		return str.size();
	}

	template <typename T>
	[[nodiscard]] constexpr T max_value(T a, T b) noexcept
	{
		return a > b ? a : b;
	}

	template <typename T>
	[[nodiscard]] constexpr T min_value(T a, T b) noexcept
	{
		return a < b ? a : b;
	}
