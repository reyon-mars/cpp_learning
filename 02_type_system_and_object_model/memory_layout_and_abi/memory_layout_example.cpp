...
void print_offsets_unaligned() {
    std::cout << "Unaligned offsets:"
              << " a=" << offsetof(Unaligned, a)
              << " b=" << offsetof(Unaligned, b)
              << " c=" << offsetof(Unaligned, c) << "\n";
}
...
void print_addresses(const Packed& p) {
    std::cout << "Packed member addresses:"
              << " c=" << static_cast<const void*>(&p.c)
              << " i=" << static_cast<const void*>(&p.i)
...
int main() {
    ...
    std::cout << "Packed overhead vs raw sum="
    ...
    std::cout << std::boolalpha
              << "Packed trivially_copyable="  << std::is_trivially_copyable_v<Packed>    << "\n"
    ...
    std::cout << "address=0x" << std::hex
              << reinterpret_cast<std::uintptr_t>(&a16) << std::dec << "\n"
              << "16-byte aligned="
              << std::boolalpha << (reinterpret_cast<std::uintptr_t>(&a16) % 16 == 0) << "\n";
